...
/**
 * @brief Powers of 10 from 10^0 to 10^9
 * This numeric type guarantees 32 bits, 
 * the value is unsigned, meaning that the range of values goes from 0 to 2^32 - 1.
 */
uint32_t tenPowers[10] = {1,10,100,1000,10000,100000,1000000,10000000,100000000,1000000000,};

/**
 * @brief Gives power of 10 at certain exp
 * 
 * @param exp 
 * @return uint32_t 
 */
 ...
 */
void radixsort_serial(uint32_t *input_array, uint32_t input_array_size,
					uint32_t max_digits_size, int debug){
	uint32_t digit;
	int f, i, j;
	double start_time;
	...
	int shared_buckets[max_digits_size][N_DIGITS];
	for (j=0; j<max_digits_size; j++)
		for(f=0; f<N_DIGITS; f++)
	...
	for (i=0; i<input_array_size; i++) {
		...
		for (j=0; j<max_digits_size; j++) {
			 ...
			 */
			f=input_array[i]/digit%N_DIGITS;
			shared_buckets[j][f]++;
		}
	}
	if (debug > 0) {
	...
	for (j=0; j<max_digits_size; j++) {
		...
		for(i=input_array_size-1;i>=0;i--) {
			int unit = input_array[i]/digit%N_DIGITS;
...
int main(int argc, char **argv)
{
	...
	start_time = omp_get_wtime();
	radixsort_serial(input, num_of_values, max_digits_size, debug);
	if (debug > 0)
		printf("K\t\tN\tTIME\n");
